/**
 * app-ophim.js — Nguồn DUY NHẤT: Ophim (ax)
 * Tối ưu: IntersectionObserver lazy-load, LRU cache, AbortController, requestIdleCallback
 */
...
// ==================== SOURCE: OPHIM ONLY ====================
const SRC = {
  name: 'Ophim', code: 'ax', CDN: 'https://img.ophim.live/uploads/movies/',
  url: {
    default: p => `https://ophim1.com/v1/api/danh-sach/phim-moi-cap-nhat?page=${p}`,
    genre:   (s,p) => `https://ophim1.com/v1/api/the-loai/${s}?page=${p}`,
    country: (s,p) => `https://ophim1.com/v1/api/quoc-gia/${s}?page=${p}`,
    year:    (y,p) => `https://ophim1.com/v1/api/nam-phat-hanh/${y}?page=${p}`,
    type:    (t,p) => `https://ophim1.com/v1/api/danh-sach/${t}?page=${p}`,
    search:  s     => `https://ophim1.com/v1/api/phim/${s}`,
    keyword: (k,p) => `https://ophim1.com/v1/api/tim-kiem?keyword=${encodeURIComponent(k)}&page=${p}`,
  },
  parse: {
    list:    d => d?.data?.items || [],
    search:  d => d?.data?.item ? [d.data.item] : [],
    keyword: d => d?.data?.items || [],
  }
};

// ==================== LRU CACHE (max 60 entries) ====================
const LRU_MAX = 60;
...
const cache = {
  ...
  set(k, v) {
    if (_cache.size >= LRU_MAX) _cache.delete(_cache.keys().next().value);
    _cache.set(k, v);
  }
};
...
const buildUrl = (mode, filter, page, genre, country, year, isSearch, isKeyword) => {
  ...
  if (mode === 'combined') {
    if (genre)   { const u = new URL(`https://ophim1.com/v1/api/the-loai/${genre}`); if (country) u.searchParams.set('country', country); if (year) u.searchParams.set('year', year); u.searchParams.set('page', page); return u.toString(); }
    if (country) { const u = new URL(`https://ophim1.com/v1/api/quoc-gia/${country}`); if (year) u.searchParams.set('year', year); u.searchParams.set('page', page); return u.toString(); }
    if (year)    return SRC.url.year(year, page);
  }
  return SRC.url.default(page);
};

const parseItems = (d, isSearch, isKeyword) => {
  const parser = isKeyword ? SRC.parse.keyword : isSearch ? SRC.parse.search : SRC.parse.list;
  return (parser(d) || []).map(it => {
    ...
    const ep = getEpisodeDisplay(it);
    return { name: it.name || it.origin_name || 'Không rõ', thumb_url: thumb, episodeDisplay: ep, slug: it.slug || it._id || '', year: String(it.year || ''), lang: (it.lang || '').toUpperCase(), quality: it.quality || '' };
...
const fetchMovies = async (mode, filter, page, genre, country, year, isSearch = false, isKeyword = false, signal = null) => {
  const key = `ax-${mode}-${filter}-${page}-${genre}-${country}-${year}-${isSearch}-${isKeyword}`;
  ...
  try {
    ...
    const result = parseItems(d, isSearch, isKeyword);
    cache.set(key, result);
    return result;
  } catch (e) { if (e.name !== 'AbortError') console.warn('[ax] fetch err:', e); return []; }
};
...
const getEpisodeDisplay = it => {
  ...
  let disp = c && t ? `Tập ${c}/${t}` : c ? `Tập ${c}` : /full|hoàn tát|hoàn thành/i.test(cur) ? 'Hoàn tất' : cur ? 'Đang phát' : '';
  if (l > 0) disp += ` (${l} link)`;
  return disp;
};

// ==================== INTERSECTION OBSERVER (lazy img) ====================
...
const createCard = m => {
  ...
  c.onclick = () => { saveState(); location.href = `detail.html?slug=${m.slug}&source=ax`; };
...
const renderMovies = (movies, container) => {
  container.innerHTML = '';
  if (!movies.length) {
    container.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:80px;color:#aaa;">Không tìm thấy kết quả. Hãy thử từ khoá khác.</div>';
    return;
  }
...
const showSec = sec => {
  ...
  if (document.getElementById(sec.id)) { document.getElementById(sec.id).style.display = 'block'; cont.insertBefore(document.getElementById(sec.id), cont.firstChild); }
...
const load = async (m, f = null, p = 1, g = null, c = null, y = null) => {
  ...
  const sid = `${m}-${f || ''}-${currentGenre || ''}-${currentCountry || ''}-${currentYear || ''}-sec`.replace(/--+/g, '-').replace(/-sec$/, '-sec');
  let sec = document.getElementById(sid);
...
const loadTxt = async () => {
  try {
    ...
    // Render nhóm tuần tự để không chặn main thread
    for (const [name, items] of Object.entries(groups)) {
      if (items.length) await loadGroup(name, items);
    }
  } catch {
...
const parseTxt = txt => {
  ...
  lines.forEach(l => {
    ...
    if (!cur) return;
    const p2 = l.split('|'); if (p2.length < 2) return;
    const [st, sk] = p2.map(x => x.trim());
    // Chỉ nhận nguồn ax (ophim)
    if (sk.toLowerCase() !== 'ophim' && sk.toLowerCase() !== 'ax') return;
...
const openModal = (() => {
  ...
  const closeModal = () => bd()?.classList.remove('show');
  let _open;
  _open = (title, items, cb) => {
    const backdrop = bd(); if (!backdrop) return;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (currentSearchQuery) {
    const inp = document.getElementById('nav-search-input');
    if (inp) inp.value = currentSearchQuery;
            ...
            if (choice === 'Chỉ lọc thể loại') { load('genre', gSlug, 1); mb?.classList.remove('show'); }
              ...
              openModal('QUỐC GIA', ['Tất cả', ...Object.keys(COUNTRY_SLUG_MAP)], cName => {
                ...
                openModal('NĂM', ['Tất cả', ...Array.from({ length: 25 }, (_, i) => String(2026 - i))], yStr => {
                  load('combined', null, 1, gSlug, cSlug, yStr === 'Tất cả' ? null : yStr);
        ...
        openModal('THỂ LOẠI', Object.keys(GENRE_SLUG_MAP), gName => {
          ...
          openModal('LỌC KẾT HỢP', ['Chỉ lọc thể loại', 'Lọc kết hợp'], choice => {
            if (choice === 'Chỉ lọc thể loại') { load('genre', gSlug, 1); mb?.classList.remove('show'); }
              ...
              openModal('QUỐC GIA', ['Tất cả', ...Object.keys(COUNTRY_SLUG_MAP)], cName => {
                openModal('NĂM', ['Tất cả', ...Array.from({ length: 25 }, (_, i) => String(2026 - i))], yStr => {
                  load('combined', null, 1, gSlug, cName === 'Tất cả' ? null : COUNTRY_SLUG_MAP[cName], yStr === 'Tất cả' ? null : yStr);
                  mb?.classList.remove('show');
                });
              });
            }
