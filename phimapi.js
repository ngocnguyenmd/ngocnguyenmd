/**
 * app-phimapi.js — Nguồn DUY NHẤT: Phimapi (bx)
...
const SRC = {
  ...
  url: {
    default: p => `https://phimapi.com/danh-sach/phim-moi-cap-nhat-v3?page=${p}`,
    genre:   (s,p) => `https://phimapi.com/v1/api/the-loai/${s}?page=${p}`,
    country: (s,p) => `https://phimapi.com/v1/api/quoc-gia/${s}?page=${p}`,
    year:    (y,p) => `https://phimapi.com/v1/api/nam/${y}?page=${p}`,
    type:    (t,p) => `https://phimapi.com/v1/api/danh-sach/${t}?page=${p}`,
    search:  s     => `https://phimapi.com/phim/${s}`,
    keyword: (k,p) => `https://phimapi.com/v1/api/tim-kiem?keyword=${encodeURIComponent(k)}&page=${p}`,
  },
  parse: {
    list:    d => d?.data?.items || d?.items || [],
    search:  d => d?.movie ? [d.movie] : [],
    keyword: d => d?.data?.items || [],
  },
  // CDN dynamique: extrait depuis la réponse API
...
const buildUrl = (mode, filter, page, genre, country, year, isSearch, isKeyword) => {
  ...
  if (mode === 'combined') {
    // Phimapi hỗ trợ thêm params country và year vào url
...
const fetchMovies = async (mode, filter, page, genre, country, year, isSearch = false, isKeyword = false, signal = null) => {
  const key = `bx-${mode}-${filter}-${page}-${genre}-${country}-${year}-${isSearch}-${isKeyword}`;
  ...
  try {
    ...
    const cdn = SRC.getCdn(d);
    const items = (parser(d) || []).map(it => {
      let thumb = it.poster_url || it.thumb_url || it.poster || it.thumb || '';
    ...
    cache.set(key, items);
    return items;
  } catch (e) { if (e.name !== 'AbortError') console.warn('[bx] fetch err:', e); return []; }
};
...
const createCard = m => {
  ...
  c.onclick = () => { saveState(); location.href = `detail.html?slug=${m.slug}&source=bx`; };
  ...
  sourceTag.className = 'movie-source-tag'; sourceTag.textContent = 'bx';
...
const parseTxt = txt => {
  ...
  lines.forEach(l => {
    ...
    // Chỉ nhận nguồn bx (phimapi)
    if (sk.toLowerCase() !== 'phimapi' && sk.toLowerCase() !== 'bx') return;
